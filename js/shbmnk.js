...
if (window.console) console.log('ШБМНК.js версии 1.4 загрузилось!'); // Если активна консоль отладки браузера, показать текст
    ...
    $("button").click(function(){ // Если была нажата кнопка
		...
		// Поменять картинки
		$('body').css('background-image', 'url(../images/theme/back.png)'); // Поменять общий фон на окулистический
		$('#left').css('background-image', 'url(../images/theme/left-body.png)'); // Поменять левый фон на окулистический
		$('#footer p').css('background-image', 'url(../images/theme/footerback.jpg)'); // Поменять подвальный фон на окулистический
		$('#lesopark').attr('src','images/theme/lesopark.png'); //Поменять лесопарк на ЧБ
		$('#borago').attr('src','images/theme-okulist/bogaro.gif'); //Поменять бораго на простое
    });
});
